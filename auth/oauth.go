...
func ConfigOauth() {
	...
	SpotifyOauthConfig = &oauth2.Config{
		...
		Scopes:       []string{"user-read-email user-top-read user-follow-read"},
...
func Routes(r *gin.RouterGroup, db *database.DB) {
	r.GET("/login", handleSpotifyLogin(db))
	r.GET("/callback", handleSpotifyCallback(db))
	r.GET("/logout", handleSpotifyLogout(db))
...
func handleSpotifyLogin(db *database.DB) gin.HandlerFunc {
...
func handleSpotifyCallback(db *database.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		session.Values["SpotifyID"] = userData.SpotifyID
...
func handleSpotifyLogout(db *database.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
...
func getAccount(db *database.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if session.ID != "" {
			...
			SpotifyID := session.Values["SpotifyID"]
			SpotifyIDStr := fmt.Sprintf("%v", SpotifyID)
			if err != nil {
				database.CheckDBErr(err.(*pq.Error), c)
				return
			}

			userData := Account{EmailStr, NameStr, PictureUrlStr, SpotifyIDStr}
